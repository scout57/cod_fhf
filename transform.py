...
available_columns = [
    ...
    "smart_242_raw",
    "smart_244_normalized",
    "smart_244_raw",
    "smart_245_normalized",
    "smart_245_raw",
    "smart_246_normalized",
    "smart_246_raw",
...
# Функция для разделения по "Модель диска" и экспорт в конец .csv файла
def apply(file):
    df = pd.read_csv(file)
    ...
    for name, data in df.groupby('model'):
        output_path = DB_FOLDER + '/' + name + '.csv'
        data[data.columns.intersection(available_columns)].to_csv(output_path, index=False, mode='a', header=not os.path.exists(output_path))
...
def transform(path):
    ...
    if not csv_files:
        raise FileNotFoundError(f"Не найдено файлов .csv по маске: {mask}.")
    
    print(f"Началось формирование базы данных из {mask}")
    ...
    for file in csv_files:
        apply(file)
        i = i + 1
        print(f"Трансформировано файлов: {i} из {total}")
